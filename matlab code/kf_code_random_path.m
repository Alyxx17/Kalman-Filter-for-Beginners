...
%% 实际轨迹（直线）
rng(17);  % 随机种子，可任意改变曲线形状

% 生成随机控制点
n_points = 10;
t_control = linspace(0, total_time, n_points);
x_control = cumsum(randn(1, n_points)) * 3;  % 随机游走
y_control = cumsum(randn(1, n_points)) * 3;

% 样条插值得到平滑路径
x_true = spline(t_control, x_control, t);
y_true = spline(t_control, y_control, t);
