...
%% 实际轨迹（螺旋线）
radius = 5;
omega = 2*pi/total_time;  % 转两圈
v_forward = 2;  % 前进速度

x_true = radius * cos(omega * t) + v_forward * t;
y_true = radius * sin(omega * t);

% 真实速度
vx_true = -radius * omega * sin(omega * t) + v_forward;
vy_true = radius * omega * cos(omega * t);

% 真实加速度
ax_true = -radius * omega^2 * cos(omega * t);
ay_true = -radius * omega^2 * sin(omega * t);
